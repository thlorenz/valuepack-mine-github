...
var batch = module.exports = function (db, res, username, cb) {
  var subgithub = sublevels(db).github
    , data
    ;

  try {
    data = structure(res, username);
    if (!data) { 
      log.warn('store-github-repos', 'Most likely response was empty because structuring returned nothing for', res);
      return cb();
    }
  } catch (e) { return cb(e); }

...
var util              =  require('util')
  ...
  , ReposToBatchTransform = require('./ReposToBatchTransform')
  , SyncBatchWriter = require('./SyncBatchWriter')
  ;
...
var go = module.exports = function (db, logins, skipExisting) {
  ...
  function start (logins_) {
    ...
    var batchTransform  =  new ReposToBatchTransform(db, { events: events });
    var syncWriter = new SyncBatchWriter(db);
    ...
    //  .pipe(tap(5))
      .pipe(syncWriter)
      .on('close', events.emit.bind(events, 'closed'))
  }
...
if (!module.parent) {
  ...
  leveldb.open(function (err, db) {
    ...
    var logins = ['danielmoore'] //['domenic', 'thej', 'thlorenz', 'danielmoore' ];
    var events = go(db, logins);

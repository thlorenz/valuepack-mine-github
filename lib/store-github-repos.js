...
function writeRepoBatch (repos, githubRepos, chunker) {
...
function writeBatch (data, username, subgithub, chunker, cb) {
  ...
  write({ type: 'put' , prefix: subgithub.usersMeta, key: meta.name, value: meta });

  if (data.reposModified) {
    writeRepoBatch(data.repos, subgithub.repos, chunker);
  ...
  if (data.starredModified && data.starredData) {
    chunker.write({ type: 'put', prefix: subgithub.starred, key: username, value: data.starredData});
  }

  if (!data.userModified) return cb();
  ...
  if (user.id) {
    write({ type: 'put' , prefix: subgithub.users, key: username, value: user });
    return cb();
  }
  ...
  subgithub.users.get(user.name, function (err, u) {
    ...
    write({ type: 'put' , prefix: subgithub.users, key: user.name, value: user });
    cb();
  })
}
...
var store = module.exports = function (db, response, username, cb) {
  ...
  writeBatch(data, username, subgithub, chunker, function (err) {

...
var github    =  require('valuepack-core/mine/namespaces').github
  ...
  , levelBatcher   =  require('level-batcher')
...
var store = module.exports = function (db, response, username, cb) {
  ...
  function storeBatch (err, batch) {
    ...
    batcher
      .pipe(through(writeBatch, endBatch))
    ...
    function writeBatch (b) {
      db.batch(b, batcher.next.bind(batcher));
    }
  ...
  createBatch(data, username, subgithub.repos, subgithub.users, subgithub.starred, subgithub.usersMeta, subgithub.byOwner, storeBatch);
};

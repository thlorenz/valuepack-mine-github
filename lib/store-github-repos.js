...
var store = module.exports = function (db, json, cb) {
  ...
  if (data.reposModified) { 
    var repoBatch = getRepoBatch(data.repos);
    tasks.push(githubRepos.batch.bind(githubRepos, repoBatch));
  }
  if (data.userModified) {
    var user = data.userData;
    tasks.push(githubUsers.put.bind(githubUsers, user.name, user));
  }

  var meta = data.userMetadata;
  tasks.push(usersMeta.put.bind(usersMeta, meta.name, meta));

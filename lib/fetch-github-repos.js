...
function userDetails (options, cb) {
  ...
  headers['If-Modified-Since'] = options.userLastModified || new Date(0)
  ...
  console.error('user headers: ', headers);
...
function repos (options, cb) {
  var headers = options.headers || {};
  headers['If-Modified-Since'] = options.reposLastModified || new Date(0)
  ...
  console.error('repos headers: ', headers);
  requestAllPages(opts, options.startPage, options.perPage, function (err, pages) {
    if (err) return cb(err);
    handlePages(pages, cb);
  })

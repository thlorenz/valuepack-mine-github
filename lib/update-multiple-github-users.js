...
var update = module.exports = function (db, users, opts) {
 ...
 // wait a minute longer by default to be safe
  ...
  function start() {
    ...
    // we may have closed the database while waiting to restart
    if (!db.isOpen()) {
      leveldb.open(db.location, function (err, db_) {
  ...
  function updateUsers() {
    ...
    updateRepos(db, nextUser, function (err, res) {
      ...
      if (smallestRemaining < minRemaining) {
        ...
        paused = true;
        // if we have to wait for a long time it makes sense to close the db in order to allow compression to occur and to inspect it
        if (minutes > 10) {
          log.info(__filename, 'wait is longer than 10 minutes, closing the database for the time being');
          leveldb.close(null, db);
        }
